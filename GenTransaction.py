...
def Transaction(limit=1, minbuy_numpro=1, maxbuy_numpro=10, min_stock_can_sell=1):
    if __name__ == "__main__":
        ...
        for (id) in cursor:
            ...
            random_product = db.query("SELECT p.id idpro, idSeller, title, stock, s.firstname SName FROM Products p JOIN Seller s on p.idSeller=s.id WHERE 1 p.in_use='{0}' ORDER BY RAND()" . format(session_code))
